...
Real,parameter::t=2./nimages,h=0.005		!-----time units per image-----time for initial pass through the RK4 routine-----!
...
integer, parameter::nxx=7*80,nyy=nxx,dampwidth=nxx/10
...
real,Dimension(nxx,nyy)::u,ux,uy,damping,rhoinv,trapwt
real,Dimension(nxx,nyy)::mu,mux,muy,lambda,lambdax,lambday
real,Dimension(nxx,0:Mmax)::Cxinv
real,Dimension(nyy,0:Nmax)::Cyinv
real,Dimension(0:Mmax,nxx)::Cx,Sx,CxinvT,gmCx
...
real,parameter::a=0.1,F0=1.,G0=1.,df=0.9,df2=19,x0=-1.,y0=2.
real,parameter::vp1=2.5,vs1=1.,rho1=2200./2200.,vp2=1.5,vs2=0.5,rho2=2200./2200.
real,parameter::mu1=vs1**2,mu2=vs2**2*rho2,lambda1=vp1**2-2.*vs1**2,lambda2=(vp2**2-2.*vs2**2)*rho2
...
call savereal('x0.out',len('x0.out'),x0)
call savereal('y0.out',len('y0.out'),y0)
...
do i=1,nxx
	do j=1,nyy
		rhoinv(i,j)=1./(rho1+(rho2-rho1)/(1.+exp(-5.*(x(i)+y(j)))))
		mu(i,j)=mu1+(mu2-mu1)/(1.+exp(-5.*(x(i)+y(j))))
		lambda(i,j)=lambda1+(lambda2-lambda1)/(1.+exp(-5.*(x(i)+y(j))))
	end do
end do
!$acc end kernels

!check that the density looks ok
!print*,maxval(rhoinv)
!print*,minval(rhoinv)
    ...
    do j=1,nyy
        do m=0,Mmax
            do n=0,Nmax
                ...
                mux(i,j)=mux(i,j)-Mmn(m,n)*m*0.5*Sx(m,i)*Cy(n,j)
                muy(i,j)=muy(i,j)-Mmn(m,n)*n*0.5*Cx(m,i)*Sy(n,j)
                lambdax(i,j)=lambdax(i,j)-Lmn(m,n)*m*0.5*Sx(m,i)*Cy(n,j)
                lambday(i,j)=lambday(i,j)-Lmn(m,n)*n*0.5*Cx(m,i)*Sy(n,j)
            end do
        end do
    end do
end do
...
do image=1,nimages
	...
	do iLoop=1,loops
		...
		do rk4step=1,4			!call the 4 steps of the rk4 routine
			...
			call SeismicRHS(AI,BI,CI,DI,Cprime,Dprime,Cx,Sx,Cy,Sy,GmCx,GnCy,nxx,nyy,trapwt,Mmax,Nmax,damping,rhoinv,mu,mux,muy,lambda,lambdax,lambday,gm,gn,dx,dy,pi)
...
Subroutine SeismicRHS(AI,BI,CI,DI,Cprime,Dprime,Cx,Sx,Cy,Sy,GmCx,GnCy,nxx,nyy,trapwt,Mmax,Nmax,damping,rhoinv,mu,mux,muy,lambda,lambdax,lambday,gm,gn,dx,dy,pi)
...
real,Dimension(nxx,nyy),intent(in)::damping,rhoinv,mu,mux,muy,lambda,lambdax,lambday,trapwt
...
real,Dimension(0:Mmax,0:Nmax)::mmA25,mnA25,nnA25,mmB25,mnB25,nnB25,mAI,nAI,mBI,nBI
...
do m=0,Mmax
    do n=0,Nmax
		...
		mAI(m,n)=AI(m,n)*m*0.5
		nAI(m,n)=AI(m,n)*n*0.5
		mBI(m,n)=BI(m,n)*m*0.5
		nBI(m,n)=BI(m,n)*n*0.5
	end do
end do
!$acc end kernels
